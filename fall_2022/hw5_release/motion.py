...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        v = np.dot(np.linalg.inv(A.T.dot(A)), A.T.dot(b))
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        A0 = Ix[y1-w: y1+w+1, x1-w: x1+w+1]
        A1 = Iy[y1-w: y1+w+1, x1-w: x1+w+1]
        G = np.array([[np.sum(A0**2), np.sum(A0*A1)], [np.sum(A0*A1), np.sum(A1**2)]])
        ...
        for k in range(num_iters):
            ...
            bk = np.array([np.sum(Ik*A0), np.sum(Ik*A1)])
            vk = G_inv.dot(bk)
            ### END YOUR CODE
